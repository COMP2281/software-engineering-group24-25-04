...
| Software Development | Data security breach | There are different companies that will be using this solution. Companies should only be able to edit their own goals. If another company edited/deleted other companies' goals, then it could result in a halt in development. | Unlikely | High | 8 | Ensure permissions work so that unauthorised staff members do not edit other companies' goals (only managers have access to edit permissions). |
