...
**3. Non-commercial options: app, or website**. Both allow tailored customisation towards our client. Having an app, uses a local database, which allows consistent, fast load times, and once downloaded, it can be used anywhere. Another feature apps offer is push notifications. But after discussions with our client, this isn’t required. Having a website, however, is favoured by our client, as per the brief. Websites can be loaded on any device, provided there’s an internet connection and doesn’t require downloading. It allows access from a range of platforms, again requested in the brief. However, non-commercial options take time to learn the specific skills required and take time to build the solution. 
...
- Finish test plan - 7th February (Sprint #3)
